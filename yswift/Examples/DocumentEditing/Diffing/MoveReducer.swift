...
struct MoveReducer<T: DiffAware> {
    func reduce(changes: [Change<T>]) -> [Change<T>] {
        ...
        inserts.forEach { insert in
            if let insertIndex = changes.firstIndex(where: { compareContentWithOptional($0.insert?.item, insert.item) }),
               let deleteIndex = changes.firstIndex(where: { compareContentWithOptional($0.delete?.item, insert.item) })
            {
